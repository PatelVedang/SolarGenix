import re
from django.db.models import Q, QuerySet, Model, Field, F
from django.http.request import QueryDict
from typing import Type
from ast import literal_eval

def filter_model(query_params: QueryDict, queryset: QuerySet[Model], model: Type[Model]):
    """
    ...
    if search_param:
        search_fields = [f.name for f in model._meta.get_fields() if isinstance(f, Field) and not f.is_relation]
        search_q = Q()
        ...
        if key not in ["sort", "search"]:
            ...
            if any(f.name == field for f in model._meta.get_fields()):
                ...
                if lookup_type == 'not_in':
                    ...
                    lookup = (
                        [val.strip() for val in value.split(',') if val.strip()]  # Split values by comma and strip whitespace
                        if isinstance(value, str) and lookup_type == 'in'
                        else [literal_eval(val) if isinstance(val, str) and val.startswith("[") else val for val in value]
                        if isinstance(value, list)
                        else literal_eval(value) if isinstance(value, str) and value.startswith("[")
                        else value
                    )
                    filter_q &= Q(**{filter_expr: lookup})
    
    if select_fields:
        select_fields = [field.strip()  for field in select_fields.split(',') if field.strip()]
