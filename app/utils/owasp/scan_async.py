...
class Scanner:
    ...
    async def execute_scans(self, url):
        try:
            try:
                options = Options()
                ...
                raise Exception('error while booting selenium on url {}'.format(url))
                pass
            
            retval = {}
            anticlickjacking = await antiClickJacking(url)
    ...
    async def main(self, directoryList):
        start = datetime.utcnow()
        loop = asyncio.get_event_loop()
        jobs = []
        for url in directoryList:
            # jobs.append(self.execute_scans(url))
            jobs.append(self.test(url))
        diff = round((datetime.utcnow()-start).total_seconds())
        try:
            await asyncio.wait_for(asyncio.gather(*jobs), timeout=10-diff)
        except asyncio.TimeoutError as e:
            raise Exception("Timeout Error occur in async")

    def process_data(self, url, order_id, user_id):
        directoryList = [url]
        ...
        loop.run_until_complete(self.main(directoryList))
        loop.close()
        output = {
