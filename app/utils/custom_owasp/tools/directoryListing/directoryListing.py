...
class directoryListing:
    ...
    def get_dir_structure(self, url, order_id, user_id):
        website = url
        protocol = ''
       ...
       # List to store all directories
        ...
        for dirpath, dirnames, filenames in os.walk(path_to_save):
            for dirname in dirnames:
                all_files.append(os.path.join(dirpath, dirname).replace(f'{path_to_save}/', protocol, 1))
            for filename in filenames:
                # Append only directories, remove the prefix
                all_files.append(os.path.join(dirpath, filename).replace(f'{path_to_save}/', protocol, 1))
...
def listDirectory(url, order_id, user_id):
    lister = directoryListing()
    return lister.get_dir_structure(url, order_id, user_id)
