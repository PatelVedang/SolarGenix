...
class Vulnerability():
    ...
    def scan(self):
        ...
        output = subprocess.getoutput(f'nmap {self.IPAddress} --host-timeout 30s')
        search = '(?P<port>\d{1,4}/tcp)\s+(?P<state>(filtered|open|closed))'
        self.ports = list(re.finditer(search, output))

        
    def check_port_status(self, log, port):    
        port_running = 0
        try:
            for port_text in self.ports:
                
                port_obj = port_text.groupdict()
                if port_obj['port'].split('/')[0] == port:
                    if port_obj:
